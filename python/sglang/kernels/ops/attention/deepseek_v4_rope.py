...
def apply_rotary_emb_triton(
    ...
    if _USE_BATCHED_ROPE:
        ...
        if positions is not None:
            # Scalar compare, not `positions.shape == (batch_size,)`: the tuple form
            # goes through Dynamo's cmp_eq polyfill, which needs a concrete bool and
            # so specializes the token dimension to the traced size. Comparing the
            # dimension directly keeps both sides symbolic.
