...
# load_jit() modules expose their entry point as a `Function` instance, and Dynamo
# cannot trace `Function.__call__`. The piecewise prefill backend compiles fullgraph,
# so that break is fatal rather than a benign split; a custom op makes the call
# opaque, the same treatment fp8_wo_a.py applies to its own JIT kernel. Deliberately
# not a @register_split_op: these kernels are small, and a split point per kernel
# would shred the graph and give back the speedup piecewise exists to provide.
