...
def _jit_compress_forward_custom_op(
    ...
    # Arity differs by plan type: CompressorDecodePlan carries one plan tensor,
    # CompressorPrefillPlan two -- so plan_b stays optional.
    if plan_b is None:
...
def compress_forward(
    ...
    *,
    ...
    if _is_xpu:
        ...
        # NamedTupleVariable and aborts the trace on data-dependent branching.
        # is_decode is a per-class constant, so the isinstance check is equivalent.
