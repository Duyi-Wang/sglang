...
class C4IndexerBackendMixin:
    ...
    def forward_c4_indexer(
        ...
        # Kept as a branch chain on purpose. Collapsing it into one F.pad (negative
        # padding truncates) removes a token-dimension specialization, but it also
        # returns a fresh tensor in the already-matching case and accuracy collapses
        # -- something downstream relies on getting the same tensor back.
