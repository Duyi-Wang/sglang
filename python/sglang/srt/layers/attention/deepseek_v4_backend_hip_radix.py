...
# Fill value for the tail rows of each metadata tensor, and the canonical statement
# of what a padded row means on this backend. 0 is the inert slot: request-pool row 0
# is reserved (free_slots starts at 1) and req_to_token is zero-initialised, so it
# resolves to the KV pool's null slot. Anything not listed below defaults to 0.
_METADATA_PAD_VALUES = {
    ...
    "swa_page_indices": -1,
    "c128_page_indices": -1,
    # flash_mla requires the clamp1 lengths to be >= 1. The *_raw lengths keep 0,
    # which is what the indexer and top-k need to take their trivial path.
    "swa_topk_lengths": 1,
...
def _refresh_metadata_rows_in_place(dst, src, _depth: int = 0) -> None:
    ...
    dst is the metadata object the captured graph recorded addresses for, so every
    tensor has to stay put; only the contents may change. src is shorter whenever the
    served batch does not fill the bucket, and the tail matters: the captured graph
    runs the whole bucket, so those rows are executed. Repeating src's last row points
    them at a real request's state and corrupts it -- fill from _METADATA_PAD_VALUES
    instead. Tensors are matched by attribute name.
    """
    ...
    for name in list(vars(dst)):
        ...
        if isinstance(d, torch.Tensor) and isinstance(s_, torch.Tensor):
            ...
            # CompressorPrefillPlan is a NamedTuple: not a Tensor and, unlike the
            # dataclasses around it, without a __dict__, so both branches above miss
            # it. Left unrefreshed, every replay runs the compressor against the
            # capture batch's plan -- all indices in bounds, so no crash, just the
            # wrong source rows on every layer.
...
def _refresh_namedtuple_rows_in_place(dst, src, _depth: int = 0) -> None:
    ...
    The tuple is immutable but its tensors are not, and it is the tensors the
    captured graph recorded addresses for. Non-tensor fields (compress_ratio,
    pin_buffer) are capture-time constants and left alone.
    """
    if _depth > 4:
        return
...
class DeepseekV4HipRadixBackend(
    ...
    # DSv4 opts into the captured-metadata contract (PrefillCudaGraphRunner selects
    # it): capture builds one metadata object per bucket and replay refreshes it in
    # place, so the addresses the captured kernels recorded stay valid and Dynamo's
    # shape guards keep holding. The CUDA backend has always done this; without it,
    # replaying a captured graph here reads stale metadata.
    ...
    def prepare_forward_metadata_for_breakable_cuda_graph_replay(
        ...
        *,
        ...
        Build against the padded batch so the shapes line up with what capture
        recorded, then copy the contents into the captured object rather than swapping
        it out -- the recorded kernels read those addresses. Mirrors
        ...
        # rarely fills the bucket. copy_ would raise on every replay and the fallback
        # would rebuild the metadata, losing the address stability that is the whole
        # point. Row-copy instead; _METADATA_PAD_VALUES covers the tail.
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch) -> None:
        ...
        # other CUDA-graph paths (decode, target-verify, out-graph) already pin it to
        # MAX_SEQ_LEN_FOR_CAPTURE; do the same while capturing or replaying piecewise.
        max_seq_len = (
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            # token count: plans trimmed to their live length (plan_c_dev[:num_c])
            # move with every batch and fail Dynamo's shape guard. The plan kernel
            # already implements the padding behind its use_cuda_graph flag.
            metadata = self.init_forward_metadata_prefill(
                ...
                # offset == num_tokens against the summed extend lengths, and does the
                # CUDA-graph padding itself from out_cache_loc.
    ...
    def _forward_unified_kv(
        ...
        *,
        ...
        # symbols and would resolve the slice by specializing the token dimension;
        # torch._check supplies the bound instead. `==` would be wrong: positions can
        # be a padded buffer while T is the live count, and telling Dynamo they match
        # folds the slice away and lets the padding ride into RoPE.
        ...
        if _cp_active:
            ...
            def _cp_round_robin(v: torch.Tensor) -> torch.Tensor:
                # CP-v2 may have fewer logical rows than its padded layout.
            ...
            # pf_* arrive padded to the graph bucket (the captured-metadata contract
            # holds their row count fixed) while q/positions do not -- DSv4 attention
            # is a split op that runs eager and truncates to the live token count.
            # Slice back to it: build_prefill_indices sizes its grids off
            # state_slot.shape[0], and _prefill_lengths_kernel's unmasked load of
            # positions past the live count corrupts the indptr for the real rows too.
        ...
        if save_kv_cache:
            ...
            # The slice is real: kv can be longer than state_slot, and dropping it
            # walks the kernel off the end. It needs a hint though -- Dynamo treats
            # n_real and kv.shape[0] as unrelated symbols and would resolve the slice
            # by specializing the token dimension. torch._check supplies the bound.
    ...
    def expand_prefill_casually(
        ...
        if padded_num_tokens is not None and padded_num_tokens > num_tokens:
            ...
            # writes. A constant also drops a .item() device sync.
