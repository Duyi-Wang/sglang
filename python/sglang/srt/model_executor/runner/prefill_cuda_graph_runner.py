...
class PrefillCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def __init__(self, model_runner: ModelRunner):
        ...
        # place at replay. TC_PIECEWISE needs it because attention layers read metadata
        # off the global forward context, where Dynamo guards it by shape -- rebuilding
        # per batch moves those shapes, and the recompiled entry holds no CUDA graph, so
        # prefill silently runs eager. NO_CAPTURED_METADATA=1 restores the eager path.
