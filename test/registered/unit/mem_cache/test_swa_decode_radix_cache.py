...
class _SWADecodeRadixScenarios:
    ...
    def test_released_prefix_survives_the_first_cache_write(self):
        """Regression: dropping a matched prefix at admission must leave the
        request in the state a miss would have left it in.

        ``DecodePreallocQueue.pop_preallocated`` can hand back the lock
        ``_match_prefix_and_lock`` took -- when the post-lock budget rejects the
        hit and the request is retried as a miss, or when the prefix reaches
        into the sliding window -- and then admit the request anyway with
        ``prefix_len=0``. From there the request runs the ordinary decode path,
        and ``cache_finished_req`` releases ``req.last_node`` once when the
        request completes (``_dec_req_lock``; this test reaches the same
        ``_dec_req_lock`` through ``cache_unfinished_req``, the cheaper of the
        two cache writes to drive). That release is only balanced if the state left
        behind is the same state a genuine miss leaves: ``last_node`` pointing
        at the (lock-free) root, so ``get_new_prebuilt_batch`` does not re-match
        and the release lands on the root as a no-op.

        Leaving the released node there -- or clearing it to ``None``, which
        makes ``get_new_prebuilt_batch`` re-match and produces an equally
        unlocked node -- ends in ``FullComponent.release_component_lock``'s
        ``assert lock_ref > 0`` on a warm tree, or, when a concurrent request
        holds that ancestor, in silently consuming *its* lock.
        """
        cache, allocator, req_to_token_pool = self._build()
        prefix_tokens = self._seq(1)
        tokens = prefix_tokens + self._seq(10**6)
        total = len(tokens)

        # A caches the prefix, so the tree owns it and B can match it.
        req_a = self._make_req(req_to_token_pool)
        req_a.origin_input_ids = array("q", prefix_tokens)
        req_a.output_ids = array("q")
        req_a.full_untruncated_fill_ids = array("q", prefix_tokens)
        req_a.set_extend_range(0, len(prefix_tokens))
        kv_a = self._alloc(allocator, len(prefix_tokens))
        req_to_token_pool.write(
            (req_a.req_pool_idx, slice(0, len(prefix_tokens))), kv_a
        )
        req_a.kv_committed_len = len(prefix_tokens)
        req_a.last_node = cache.root_node_handle()
        req_a.cache_protected_len = 0
        req_a.extra_key = None
        req_a.kv = ReqKvInfo(kv_allocated_len=len(prefix_tokens), swa_evicted_seqlen=0)
        cache.cache_finished_req(
            req_a, is_insert=True, kv_len_to_handle=len(prefix_tokens)
        )

        # B is admitted through `pop_preallocated`: match + lock, then release.
        ...
        match_prefix_for_req(cache, req_b, req_b.origin_input_ids, include_req=True)
        self.assertGreater(
            len(req_b.prefix_indices), 0, "fixture produced no prefix to release"
        )
        lock = cache.inc_lock_ref(req_b.last_node)
        req_b.swa_uuid_for_lock = lock.swa_uuid_for_lock
        req_b.skip_lock_node_ids = lock.skip_lock_node_ids
        queue._release_prefix_lock(req_b)

        # `_pre_alloc` for the miss: B owns a private copy of the whole sequence.
        ...
        req_b.kv = ReqKvInfo(kv_allocated_len=total, swa_evicted_seqlen=0)
        full_after_prealloc = allocator.full_available_size()

        # `get_new_prebuilt_batch`: re-match only when the match state is gone.
        tree_for_batch = cache if req_b.last_node is None else None
        req_b.init_next_round_input(tree_for_batch)
        req_b.set_extend_range(len(req_b.prefix_indices), req_b.kv_committed_len)

        cache.cache_unfinished_req(req_b)  # must not raise

        # B's duplicate copy of the shared prefix is handed back to the pool;
        # a mismatch here means the insert deduped against a prefix length the
        # request does not actually own, and those slots leak.
        self.assertEqual(
            allocator.full_available_size() - full_after_prealloc,
            len(prefix_tokens),
        )
        self.assertEqual(req_b.cache_protected_len, total)
        cache.sanity_check()
